...
async def list_comments(
    ...
    try:
        result = await use_case.get(DataBase, post_id, skip, limit)
...
async def get_comment(comment_id: int, DataBase: AsyncSession = Depends(get_db)) -> CommentOut:
    logger.info(f"Запрос комментария с id={comment_id}")
    use_case = MethodsForComment()
    try:
        result = await use_case.get_detail(DataBase, comment_id)
...
async def create_comment(payload: CommentCreate, DataBase: AsyncSession = Depends(get_db),
    ...
    use_case = MethodsForComment()
    try:
        result = await use_case.create(DataBase, payload, user.id)
...
async def update_comment(comment_id: int, payload: CommentUpdate,
    ...
    use_case = MethodsForComment()
    try:
        result = await use_case.update(DataBase, comment_id, payload, user.id)
...
async def delete_comment(comment_id: int, DataBase: AsyncSession = Depends(get_db),
                   user: UserOut = Depends(get_current_user)):
    logger.info(f"Попытка удаления комментария id={comment_id} от пользователя id={user.id}")
    use_case = MethodsForComment()
    try:
        await use_case.destroy(DataBase, comment_id, user.id)
