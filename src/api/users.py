...
async def list_users(skip: int = 0, limit: int = 20, DataBase: AsyncSession = Depends(get_db),
    ...
    result = await use_case.get(DataBase, skip, limit)
...
async def get_user_any(nickname: str, DataBase: AsyncSession = Depends(get_db),
    ...
    use_case = MethodsForUser()
    try:
        result = await use_case.get_detail(DataBase, nickname)
...
async def get_user(user: UserOut = Depends(get_current_user)) -> UserOut:
...
async def create_user(payload: UserCreate, DataBase: AsyncSession = Depends(get_db)) -> UserOut:
    logger.info(f"Попытка создания пользователя с никнеймом '{payload.nickname}'")
    use_case = MethodsForUser()
    try:
        result = await use_case.create(DataBase, payload)
...
async def update_user(payload: UserUpdate, DataBase: AsyncSession = Depends(get_db),
                user: UserOut = Depends(get_current_user)) -> UserOut:
    logger.info(f"Попытка обновления профиля пользователя {user.nickname}")
    use_case = MethodsForUser()
    try:
        result = await use_case.update(DataBase, user.nickname, payload)
...
async def delete_user(DataBase: AsyncSession = Depends(get_db), user: UserOut = Depends(get_current_user)):
    logger.info(f"Попытка удаления аккаунта пользователя {user.nickname}")
    use_case = MethodsForUser()
    try:
        await use_case.destroy(DataBase, user.nickname)
