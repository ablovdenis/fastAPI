...
async def list_categories(skip: int = 0, limit: int = 10,
                    DataBase: AsyncSession = Depends(get_db)) -> List[CategoryOut]:
...
async def get_category(category_slug: str, DataBase: AsyncSession = Depends(get_db)) -> CategoryOut:
    logger.info(f"Запрос категории с slug='{category_slug}'")
    use_case = MethodsForCategory()
    try:
        result = await use_case.get_detail(DataBase, category_slug)
...
async def create_category(payload: CategoryUpdateAndCreate,
...
async def update_category(category_slug: str, payload: CategoryUpdateAndCreate,
                    DataBase: AsyncSession = Depends(get_db),
                    _: UserOut = Depends(get_current_user)) -> CategoryOut:
    logger.info(f"Попытка обновления категории slug='{category_slug}'")
    use_case = MethodsForCategory()
    try:
        result = await use_case.update(DataBase, category_slug, payload)
...
async def delete_category(category_slug: str, DataBase: AsyncSession = Depends(get_db),
                    _: UserOut = Depends(get_current_user)):
    logger.info(f"Попытка удаления категории slug='{category_slug}'")
    use_case = MethodsForCategory()
    try:
        await use_case.destroy(DataBase, category_slug)
