...
async def list_locations(skip: int = 0, limit: int = 20,
                   DataBase: AsyncSession = Depends(get_db)) -> List[LocationOut]:
...
async def get_location(name: str, DataBase: AsyncSession = Depends(get_db)) -> LocationOut:
    logger.info(f"Запрос локации по имени='{name}'")
    use_case = MethodsForLocation()
    try:
        result = await use_case.get_detail(DataBase, name)
...
async def create_location(payload: LocationUpdateAndCreate,
...
async def update_location(name: str, payload: LocationUpdateAndCreate,
                    DataBase: AsyncSession = Depends(get_db),
                    _: UserOut = Depends(get_current_user)) -> LocationOut:
    logger.info(f"Попытка обновления локации с именем='{name}'")
    use_case = MethodsForLocation()
    try:
        result = await use_case.update(DataBase, name, payload)
...
async def delete_location(name: str, DataBase: AsyncSession = Depends(get_db),
                    _: UserOut = Depends(get_current_user)):
    logger.info(f"Попытка удаления локации с именем='{name}'")
    use_case = MethodsForLocation()
    try:
        await use_case.destroy(DataBase, name)
